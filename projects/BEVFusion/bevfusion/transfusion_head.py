...
from .map_query_cross_attention import PositionEncodingLearned
...
class TransFusionHead(nn.Module):
    ...
    def __init__(
        ...
        # ✅ 在 TransFusionHead 中 __init__ 添加
        self.use_map_cross_attn = True  # config 中控制是否启用
        # ✅ 使用同一个位置编码器
        self.pos_encoder = PositionEncodingLearned(input_channel=2, num_pos_feats=128)

        # ✅ 将 pos_encoder 传给 map 交叉注意力模块
        self.map_cross_attn_module = MapQueryCrossAttention(
        ...
        # # ✅ 在 TransFusionHead 中 __init__ 添加
        # self.use_map_cross_attn = True  # config 中控制是否启用
        # self.query_pos_encoder = PositionEncodingLearned(input_channel=2, num_pos_feats=128)
        # self.map_pos_encoder = PositionEncodingLearned(input_channel=2, num_pos_feats=128)
        # self.map_cross_attn_module = MapQueryCrossAttention(
        #     embed_dim=hidden_channel,
        #     query_pos_encoder=self.query_pos_encoder,
        #     map_pos_encoder=self.map_pos_encoder
        # )
    ...
    def forward_single(self, inputs, metas):
        ...
        fusion_feat = self.shared_conv(inputs)#[B,128,H180,W180]
        # print(f"✅ fusion_feat shape----------------------------------------------: {fusion_feat.shape}")
        ...
        # # ✅ 打印 topk 的 heatmap 分布
        # topk_scores = heatmap.view(batch_size, -1).gather(1, top_proposals)

        # # 打印统计量
        # for b in range(min(batch_size, 2)):  # 打印前2个batch
        #     print(f"�� Batch {b}: Top-{self.num_proposals} proposal scores:")
        #     print(f"  ⤷ max: {topk_scores[b].max().item():.4f}")
        #     print(f"  ⤷ min: {topk_scores[b].min().item():.4f}")
        #     print(f"  ⤷ mean: {topk_scores[b].mean().item():.4f}")
        #     print(f"  ⤷ median: {topk_scores[b].median().item():.4f}")
        #     print(f"  ⤷ top-10: {topk_scores[b][:10].cpu().numpy()}")
        ...
        # ✅ 在 TransFusionHead 中 forward_single 插入（在 query_feat + 类别编码之后）
        map_mask_bev = getattr(self, 'map_mask_bev', None)
        # print(f"✅ map_mask_bev shape: {map_mask_bev.shape}")
        # 假设 fusion_feat.shape[-2:] = H, W
        map_pos = self.bev_pos.repeat(batch_size, 1, 1).to(fusion_feat.device)


        if self.use_map_cross_attn and self.map_mask_bev is not None:
            map_mask_bev = self.map_mask_bev
            if map_mask_bev.shape[-2:] != fusion_feat.shape[-2:]:
                map_mask_bev = F.interpolate(map_mask_bev, size=fusion_feat.shape[-2:], mode='nearest')
                H_new, W_new = fusion_feat.shape[-2:]
                new_grid_x = torch.linspace(0, H_new - 1, H_new, device=fusion_feat.device) + 0.5
                new_grid_y = torch.linspace(0, W_new - 1, W_new, device=fusion_feat.device) + 0.5
                new_grid_x, new_grid_y = torch.meshgrid(new_grid_x, new_grid_y, indexing='ij')
                new_pos = torch.stack([new_grid_x, new_grid_y], dim=0).reshape(2, -1).T  # [HW, 2]
                map_pos = new_pos.unsqueeze(0).repeat(batch_size, 1, 1)  # [B, HW, 2]

            query_feat = self.map_cross_attn_module(
                query_feat,
                map_mask_bev,
                query_pos=query_pos,
                map_pos=map_pos
