...
def to_tensor(data):
    """Convert `numpy.ndarray` or `list` to `torch.Tensor`."""
    if isinstance(data, torch.Tensor):
        ...
        raise TypeError(f"Unsupported data type {type(data)} for to_tensor.")
...
class Det3DDataPreprocessor(DetDataPreprocessor):
    ...
    def simple_process(self, data: dict, training: bool = False) -> dict:
        ...
        # print("---------------------------------------------------------------")
        # print(batch_inputs)
        # if 'map_mask' in inputs:
        #     print("33333-------------------------")
        if 'map_mask' in inputs:
            # print(f"🚀 Before converting, map_mask type: {type(inputs['map_mask'])}")
            batch_inputs['map_mask'] = torch.stack([to_tensor(m) for m in inputs['map_mask']], dim=0)
            # print(f"✅ After converting, map_mask type: {type(batch_inputs['map_mask'])}")
        ...
        # print(f"🚀 Final batch_inputs keys before return: {batch_inputs.keys()}")
