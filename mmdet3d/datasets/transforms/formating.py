...
class Pack3DDetInputs(BaseTransform):
    INPUTS_KEYS = ['points', 'img','map_mask']
    ...
    def pack_single_results(self, results: dict) -> dict:
        ...
        """
        # Format 3D data
        # print(f"Before packing: {results.keys()}")
        if 'map_mask' in results:
            # 如果不是 tensor，则转换
            if not isinstance(results['map_mask'], torch.Tensor):
                results['map_mask'] = to_tensor(results['map_mask'])
            # print(f"✅ map_mask shape: {results['map_mask'].shape}")
        else:
            print("❌ map_mask missing before packing!")
        ...
        # print(f"✅ Packed inputs keys: {packed_results['inputs'].keys()}")
