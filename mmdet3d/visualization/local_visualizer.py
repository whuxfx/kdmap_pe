...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    def set_bev_image(self, points=None, bev_range=(-54, -54, 54, 54), bev_resolution=0.12):
        x_min, y_min, x_max, y_max = bev_range

        # 保存范围信息，后续bbox绘制要一致
        self.bev_x_min = x_min
        self.bev_y_min = y_min
        self.bev_resolution = bev_resolution

        bev_width = int((x_max - x_min) / bev_resolution)
        bev_height = int((y_max - y_min) / bev_resolution)
        

        bev_image = np.ones((bev_height, bev_width, 3), dtype=np.uint8) * 255
        ...
        if points is not None:
            ...
            x_img = ((points[:, 0] - x_min) / bev_resolution).astype(np.int32)
            y_img = ((points[:, 1] - y_min) / bev_resolution).astype(np.int32)

            valid_mask = (x_img >= 0) & (x_img < bev_width) & (y_img >= 0) & (y_img < bev_height)
            x_img, y_img = x_img[valid_mask], y_img[valid_mask]
            # x_img = bev_width - x_img - 1

            bev_image[y_img, x_img, :] = 0  # 点云黑色绘制

        # 顺时针旋转90°
        bev_image = np.rot90(bev_image, -1)
        ...
        self.ax_save.imshow(bev_image)

        # 保存旋转信息，方便bbox进行一致变换
        self.bev_width, self.bev_height = self.width, self.height
    ...
    def draw_bev_bboxes(self, bboxes_3d, scale=10, edge_colors='royalblue', line_widths=1, alpha=1):
        check_type('bboxes_3d', bboxes_3d, BaseInstance3DBoxes)
        ...
        # 原始坐标映射
        poly[..., 0] = (poly[..., 0] - self.bev_x_min) * scale
        poly[..., 1] = (poly[..., 1] - self.bev_y_min) * scale

        # 上下翻转（Y轴翻转）
        poly[..., 1] = self.bev_height - poly[..., 1]

        # # anchor逆时针旋转90°（关键修改在这里）
        # poly_rot = np.empty_like(poly)
        # poly_rot[..., 0] = poly[..., 1]
        # poly_rot[..., 1] = self.bev_width - poly[..., 0]
        # poly = poly_rot

        if isinstance(edge_colors, str):
            bbox_colors = [color_val_matplotlib(edge_colors)] * len(poly)
        elif isinstance(edge_colors, (tuple, list, np.ndarray)):
            bbox_colors = edge_colors / 255 if np.max(edge_colors) > 1 else edge_colors
        else:
            raise ValueError('edge_colors格式不支持')

        for i, corners in enumerate(poly):
            polygon = plt.Polygon(
                corners,
                closed=True,
                ...
                facecolor='none',
                alpha=alpha)
            self.ax_save.add_patch(polygon)
    ...
    def _draw_instances_3d(self,
        ...
        if vis_task in ['lidar_det', 'multi-modality_det']:
            ...
            colors = np.array([bbox_palette[label] for label in labels_3d]) / 255.0
        ...
        # 固定BEV区域调用方法
        bev_range = (-54, -54, 54, 54)  # 可以调整
        bev_resolution = 0.12  # 0.2米/像素，根据需要调整
        self.set_bev_image(points=points, bev_range=bev_range, bev_resolution=bev_resolution)
        self.draw_bev_bboxes(bboxes_3d, edge_colors=colors, scale=1 / bev_resolution)
        bev_img = self.get_image()
        data_3d['bev'] = bev_img
        # bev_img_cropped = crop_white_border(bev_img)
        # data_3d['bev'] = bev_img_cropped
        ...
        if vis_task in ['mono_det', 'multi-modality_det']:
            ...
            if isinstance(img, list) or (isinstance(img, (np.ndarray, Tensor))
                ...
                cam_names = input_meta['cam2img'].keys()
                cam2img = input_meta['cam2img']
                imgs_reordered = []
                cam2img_reordered = {}

                for cam in desired_order:
                    idx = list(cam_names).index(cam)
                    imgs_reordered.append(img[idx])
                    cam2img_reordered[cam] = cam2img[cam]

                img = imgs_reordered
                input_meta['cam2img'] = cam2img_reordered

                # 拼成 2 行 3 列
                img_row0 = np.concatenate([img[0], img[1], img[2]], axis=1)
                img_row1 = np.concatenate([img[3], img[4], img[5]], axis=1)
                composed_img = np.concatenate([img_row0, img_row1], axis=0)
    ...
    def show(self,
            ...
            drawn_bev_img: Optional[np.ndarray] = None,  # 新增BEV图像参数
            win_name: str = 'image',
            wait_time: int = -1,
        ...
        # 先显示投影的2D图像（如果存在）
        if drawn_img is not None or drawn_img_3d is not None:
            img_to_show = drawn_img if drawn_img is not None else drawn_img_3d
            plt.figure(figsize=(12, 8))
            plt.title('2D Projection View', fontsize=16)
        ...
        # 再显示BEV视图（如果存在）
        if drawn_bev_img is not None:
            plt.figure(figsize=(10, 10))
            plt.title('BEV (Bird\'s Eye View)', fontsize=16)
            plt.imshow(drawn_bev_img)
            plt.axis('off')
            plt.tight_layout()
            plt.show()
        ...
        if hasattr(self, 'o3d_vis'):
            ...
            if wait_time != -1:
                ...
                while (time.time() - self.last_time < wait_time
                    ...
                    self.o3d_vis.update_renderer()
                    self.view_port = self.view_control.convert_to_pinhole_camera_parameters()
    ...
    def add_datasample(self,
        ...
        """
        # # 初始化全局计数器（仅第一次调用时）
        # if not hasattr(self, 'total_sample_count'):
        ...
        # # 统计当前样本中的图像数量
        # cur_img_count = 0
        # if 'img' in data_input:
        #     if isinstance(data_input['img'], list):
        #         cur_img_count = len(data_input['img'])
        #         print_log(f"检测到多视角图像，总数：{cur_img_count}", logger='current')
        #         for i, img in enumerate(data_input['img']):
        #             if isinstance(img, np.ndarray):
        #                 print_log(f"图像 {i} 形状：{img.shape}", logger='current')
        #             else:
        #                 print_log(f"图像 {i} 不是numpy数组", logger='current')
        #     elif isinstance(data_input['img'], np.ndarray):
        #         cur_img_count = 1
        #         print_log(f"单视角图像形状：{data_input['img'].shape}", logger='current')
        # else:
        #     print_log("data_input 中未检测到 'img' 键", logger='current')
        # self.total_img_count += cur_img_count

        # # 打印累计统计信息
        # print_log(f"累计样本数：{self.total_sample_count}, 累计图像数：{self.total_img_count}", logger='current')

        # # 检查点云数据
        # if 'points' in data_input:
        #     pts = tensor2ndarray(data_input['points'])
        #     print_log(f"点云数据形状：{pts.shape}", logger='current')
        # else:
        #     print_log("data_input 中未检测到 'points' 键", logger='current')
        ...
        if draw_pred and data_sample is not None:
            ...
            # print(f"✅ 是否启用 pred 绘图: {draw_pred}")
            # print(f"✅ data_sample 是否包含 pred_instances_3d: {'pred_instances_3d' in data_sample}")
            if 'pred_instances_3d' in data_sample:
                # print(f"✅ pred_instances_3d 数量: {len(data_sample.pred_instances_3d)}")
                # print(f"✅ pred_instances_3d 分数最大值: {data_sample.pred_instances_3d.scores_3d.max().item()}")
        ...
        if show:
            self.show(
                ...
                wait_time=wait_time,
                vis_task=vis_task)
        # 在add_datasample末尾的保存部分，新增对bev视图的保存:
            # print("------------------------------------------------------------")
        ...
        if out_file is not None:
            ...
            if pred_data_3d is not None and 'bev' in pred_data_3d:
                mmcv.imwrite(pred_data_3d['bev'][..., ::-1], out_file[:-4] + '_bev' + out_file[-4:])
        else:
